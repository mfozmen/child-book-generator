...
- **P1 — Samsung-Notes duplicate-text fix.** When a page comes in as a single image that carries both the child's text and an illustration (Samsung Notes / phone-scan exports), running ``transcribe_page`` puts the text into ``page.text`` but leaves the image in place — so the renderer prints both and the text appears *twice* on the page. On confirmed OCR, clear ``page.image`` and set ``page.layout = "text-only"`` automatically. The illustration is lost, but a follow-up ``generate_page_illustration`` (under "Explicitly deferred" below) will restore it cleanly. Surface the trade-off in the confirm prompt so the user knows what's happening before they approve.
- **P2 — Skip blank pages.** Samsung Notes commonly trails 2-3 blank pages. ``transcribe_page`` correctly flags them (``<BLANK>`` sentinel, PR #47) but they stay in ``draft.pages`` and both ``propose_layouts`` and the renderer treat them as real pages. New tool ``skip_page(page: int)`` with a ``confirm`` y/n — removes the page from ``draft.pages`` so it never reaches the renderer. ``transcribe_page``'s blank-sentinel result suggests the next step explicitly.
